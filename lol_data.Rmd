...
global_data %>% 
  ...
  arrange(player_name)
```
Finally, let's throw together a table of wins and losses for the year. I'm discarding all teams flagged as "unknown" for brevity.
...
There's a hell of a lot going on in this data, and it makes sense in my mind to segment it down in some logical fashion.

There's a ton of different ways we could do it. Aggregating by team and league certainly seem most pertinent.
...
That means that for our purposes the LEC (Europe), LPL (China), LCS (North America) and LCK (South Korea) are going to be the first ports of call.

```{r create_league_data}
# Fetch the leagues and run 

leagues_list <- as.list(global_data %>% 
  group_by(league) %>% 
  summarise() %>% 
  pull())

for(i in 1:length(leagues_list)){
  # Create string to define tibble
  tibble_name = paste0(leagues_list[i],"_data")
  
  cur_league <- global_data %>% 
    filter(league == leagues_list[[i]])

  # Create the new tibble
  assign(to_snake_case(paste0(tibble_name)), cur_league)
}
...
```{r create_league_tables}
# Create win/loss tables for every league
league_losses <- list()
league_wins <- list()

for(i in 1:length(leagues_list)){
  # Create string to reference a specific tibble
  league = to_snake_case(paste0(leagues_list[i],"_data"))
  
  # Use get() to call this object by name
  league_wins[[i]] <- get(league) %>% 
  ...
  league_losses[[i]] <- get(league) %>% 
  filter(participant_id == 100 & winner == FALSE | participant_id == 200 & winner == FALSE) %>% 
  group_by(team_name) %>% 
  summarise(losses = n()) %>% 
  arrange(desc(losses))
  
  # Use assign() to create the name of our new object and fill it with data
  assign(to_snake_case(paste0(leagues_list[i],"_table")), 
         (as.data.frame(league_wins[i])) %>% 
               inner_join(as.data.frame(league_losses[i]), by = "team_name") %>% 
          mutate(played = rowSums(across(c("wins", "losses")))))
}

# Environment cleanup

rm(i, league, league_wins, league_losses)
```

LEC

```{r call_lec_table,echo=FALSE}
lec_table
```

LPL

```{r call_lpl_table,echo=FALSE}
lpl_table
```

LCS

```{r call_lcs_table,echo=FALSE}
lcs_table
```

LCK

```{r call_lck_table,echo=FALSE} 
lck_table
```
