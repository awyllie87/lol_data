...
for(i in 1:length(leagues_list)){
  ...
  assign(str_to_lower(paste0(tibble_name)), cur_league)
}
...
for(i in 1:length(leagues_list)){
  ...
  league = str_to_lower(paste0(leagues_list[i],"_data"))
  ...
  assign(str_to_lower(paste0(leagues_list[i],"_table")), 
         (as.data.frame(league_wins[i])) %>% 
               inner_join(as.data.frame(league_losses[i]), by = "team_name") %>% 
          mutate(games_played = sum(wins, losses)))
}
